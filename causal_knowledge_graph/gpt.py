...
def main(data_type, out_path):
    ...
    for num_iter in tqdm(range(100)):
        ...
        prompts_file = out_path / "prompts" / f"{data_type}-{MODEL}-prompt-{prompt_version}-{number_of_samples}-{temperature}.txt"
    ...
    out_file = out_path / "results" / f"{data_type}-{MODEL}-{prompt_version}-{number_of_samples}-{temperature}-100_gpt_responses.jsonl"
